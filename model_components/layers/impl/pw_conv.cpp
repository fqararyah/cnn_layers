...
#pragma HLS UNROLL
			...
			{
#pragma HLS UNROLL
				channels_tile[t_h][t_w] = channels[starting_index + t_d * pw_tile_hw + t_h * pw_tile_w + t_w];
			}
		}
	}
}
...
#pragma HLS INLINE off
	...
	if (tile_index >= 0)
	{
		...
		rec_scales_dt add_layer_scale_reciprocal =
			add_layers_fms_scales_rec[layer + 1];
...
//				num_of_weight_groups, layer_weights_offset);
		...
		fill_fused_zero_points_buffer(fused_zero_points,
				fused_zero_points_buffer, td_o * pw_conv_parallelism_out, layer,

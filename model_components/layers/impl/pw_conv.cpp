...
#pragma HLS UNROLL
				...
				{
					channels_tile[t_d][t_h][t_w] = channels[starting_indx + t_d * pw_tile_hw + t_h * pw_tile_w + t_w];
					//					cout
...
#pragma HLS INLINE off
	...
	if (read_write == 1 || read_write == 3)
	{
		...
		{
			skip_connection_other_layer_scale = conv_fms_scales[layer - skip_connection_depth + 1];
			skip_connection_other_layer_zero_point = conv_fms_zero_points[layer - skip_connection_depth + 1];
		}
		else
		{
			skip_connection_other_layer_scale = add_layers_fms_scales[layer - skip_connection_depth + 1];
			skip_connection_other_layer_zero_point =
				add_layers_fms_zero_points[layer - skip_connection_depth + 1];
		}

	read_a_tile_from_tmp_channels_tile_o_d:
...
#pragma HLS PIPELINE II = 2
				...
				read_a_tile_from_tmp_channels_tile_h:
					for (int t_h = 0;
						 t_h < pw_tile_h; t_h++)
					{
#pragma HLS UNROLL
					read_a_tile_from_tmp_channels_w:
						for (int t_w = 0;
							 t_w < pw_tile_w; t_w++)
						{
#pragma HLS UNROLL
							...
							tmp_channels_scaled_tile[tile_offset * pw_tile_d + t_d][t_h][t_w] =
...
void scale_pss_tile(
	...
	int starting_d, const int layer, int read_write)
{
#pragma HLS INLINE
...
#pragma HLS INLINE off
	...
	rec_scales_dt add_layer_scale_reciprocal = add_layers_fms_scales_rec[layer + 1];
...
#pragma HLS UNROLL
				pw_write_results_tile_w:
					for (int t_w = 0; t_w < pw_tile_w;
						 t_w++)
					{
#pragma HLS UNROLL
						const int to_write_at_index = current_tile_indx + t_d * pw_tile_hw + t_h * pw_tile_w + t_w;

						pss_f_dt scaled_val = scaled_result_tile[tile_offset * pw_tile_d + t_d][t_h][t_w];
						//						if (layer == 39) {
						...
						{ // 1: projection
							pss_f_dt distant_val =
								tmp_channels_scaled_tile[tile_offset * pw_tile_d + t_d][t_h][t_w];
							pss_f_dt tmp = (scaled_val + distant_val) * add_layer_scale_reciprocal + add_layer_zero_point;
							if (tmp < 0)
							{
								tmp -= quant_dw_half;
							}
...
#pragma HLS UNROLL
				...
				{
#pragma HLS UNROLL
					if (starting_conv_d + t_d < layer_conv_d && starting_filter + f_d < layer_num_fils)
					{
						tmp += channels_tile[t_d][t_h][t_w] * weights_tile[f_d][starting_conv_d + t_d];
					}
				}
...
#pragma HLS ARRAY_PARTITION variable = scaled_result_tile complete dim = 0
			...
			scale_pss_tile(results_tile, scaled_result_tile, layer_relu,
						   fused_scales_buffer, fused_scales_log_2_shifts_buffer,

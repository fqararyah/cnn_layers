...
//	const int filled_first_time = layer_0_s_filter_dim - layer_0_s_strides;
//	if (starting_h == 0) // first time
//			{
//		for (int d = 0; d < input_image_depth; d++) {
//			for (int h = 0; h < layer_0_s_filter_dim - layer_0_s_strides; h++) {
...
//				for (int w = 0; w < input_image_width; w++) {
//					channels_tile[d][h][w] =
//							channels_tile[d][h + layer_0_s_strides][w];
//				}
//			}
//		}
//	}
//
//	for (int d = 0; d < input_image_depth; d++) {
//		const int start_filling_h_offset = layer_0_s_filter_dim - layer_0_s_strides;
//		for (int h = start_filling_h_offset; h < layer_0_s_filter_dim; h++) {
//			for (int w = 0; w < input_image_width; w++) {
//				if (h + starting_h * layer_0_s_strides + filled_first_time
//						- start_filling_h_offset < input_image_height) {
//					channels_tile[d][h][w] = channels[d][h
//							+ starting_h * layer_0_s_strides + filled_first_time
					...
					for (int c_w = 0; c_w < layer_0_s_filter_dim; c_w++) {
						if (w * layer_0_s_strides + c_w < layer_0_s_ifm_width) {
							tmp += weights_0[f][d][c_h][c_w]
									* channels_tile[d][c_h][w * layer_0_s_strides
											+ c_w];
						} else {
							tmp += weights_0[f][d][c_h][c_w]
...
//					cout << "*********\n";
			...
			const int tile_index = tile_in_d
					* (layer_0_s_num_of_tiles_h * layer_0_s_num_of_tiles_w)
					+ tile_in_h * layer_0_s_num_of_tiles_w + tile_in_w;
...
void layer_0_3x3(
	...
	for (int h = 0; h < layer_0_s_ofm_height; h++) {
		...
		layer_0_conv_engine(weights_0, channels_tile, result, h,  fused_scales, fused_scales_log_2_shifts, relu_6_fused_scales, fused_zero_points);
	}
}

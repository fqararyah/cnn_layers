...
void dw_conv_fill_from_channels(fms_dt channels[max_fms_size],
		fms_dt ifms_buffer_lower_part[dw_pipeline_depth][max_strides][switch_point_fms_width
...
#pragma HLS INLINE off
	...
	dw_fill_channels_tile: for (int o_d = 0;
...
#pragma HLS UNROLL
					if (ifms_buffer_offset_h < ifms_height
						...
						ifms_buffer_lower_part[o_d * dw_tile_d + d][0][current_buffer_offset_after_o_w
								+ w] =
					...
					if (strides == 2) {
						if (ifms_buffer_offset_h + 1 < ifms_height
								&& o_w * dw_tile_w + w < ifms_width) {
							ifms_buffer_lower_part[o_d * dw_tile_d + d][1][current_buffer_offset_after_o_w
									+ w] =
...
void dw_conv_copy_to_ifm_buffer(
		fms_dt ifms_buffer_lower_part[max_strides][max_strides][switch_point_fms_width
				+ max_padding_lr],
		fms_dt ifms_buffer[dw_pipeline_depth][max_filter_hw_dim][switch_point_fms_width
				+ max_padding_lr], const int strides, const int filter_dim,
		const int ifms_width, const int padding_left_or_right) {
#pragma HLS INLINE off

	dw_shift_d: for (int d = 0; d < dw_pipeline_depth; d++) {
#pragma HLS PIPELINE II=2
		dw_shift_w: for (int w = 0; w < switch_point_fms_width + max_padding_lr;
				w++) {
#pragma HLS UNROLL
			if (w >= ifms_width) {
				break;
...
#pragma HLS UNROLL
			...
			if (strides == 2) {
				ifms_buffer[d][filter_dim - strides + 1][w
						+ padding_left_or_right] =
...
#pragma HLS ARRAY_PARTITION variable = engine_result_tile_copy type = complete dim = 2
	...
	for (int dw_pipeline_in_d = 0;
		...
		for (int h = 0; h < dw_num_of_runs_in_pipe + layer_ifm_height / strides;
				h++) {
			int read_results_h =
					h < layer_ifm_height / strides ?
							h : (layer_ifm_height / strides) - 1;
			const int absolute_offset_in_ifms = (tile_in_d
					* num_of_ifms_tiles_hw
					+ ((read_results_h * strides + rows_filled_first_time)
							/ dw_tile_h) * num_of_ifms_tiles_w) * dw_tile_size
					+ ((read_results_h * strides + rows_filled_first_time)
							% dw_tile_h) * dw_tile_w;

			const int absolute_offset_in_ifms_2 = (tile_in_d
					* num_of_ifms_tiles_hw
					+ ((read_results_h * strides + rows_filled_first_time + 1)
							/ dw_tile_h) * num_of_ifms_tiles_w) * dw_tile_size
					+ ((read_results_h * strides + rows_filled_first_time + 1)
							% dw_tile_h) * dw_tile_w;

			int write_to_results_h =
					(h - dw_num_of_runs_in_pipe) < 0 ?
							0 : (h - dw_num_of_runs_in_pipe);
			const int absolute_offset_in_results = (tile_in_d
					* num_of_ofms_tiles_hw
					+ (write_to_results_h / dw_tile_h) * num_of_ofms_tiles_w)
					* dw_tile_size

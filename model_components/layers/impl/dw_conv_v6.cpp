...
dw_fill_channels_tile:
	for (int o_d = 0; o_d < dw_pipeline_depth; o_d +=
											   dw_tile_d)
	{
		const int current_absolute_offset_after_o_d =
			global_absolute_tile_offset_in_ifms + (o_d / dw_tile_d) * num_of_tiles_hw * dw_tile_size;
		const int current_absolute_offset_after_o_d_2 =
			global_absolute_tile_offset_in_ifms_2 + (o_d / dw_tile_d) * num_of_tiles_hw * dw_tile_size;
...
#pragma HLS UNROLL
					...
					{
						ifms_buffer_lower_part[o_d + d][0][current_buffer_offset_after_o_w + w] =
							channels[current_absolute_offset_after_o_w + d * dw_tile_hw + w];
					}
					else
					{
						ifms_buffer_lower_part[o_d + d][0][o_w * dw_tile_w + w + padding_left_or_right] = fms_zero_point;
					}
					if (strides == 2)
					{
						if (ifms_buffer_offset_h + 1 < ifms_height && o_w * dw_tile_w + w < ifms_width)
						{
							ifms_buffer_lower_part[o_d + d][1][current_buffer_offset_after_o_w + w] =
								channels[current_absolute_offset_after_o_w_2 + d * dw_tile_hw + w];
						}
						else
						{
							ifms_buffer_lower_part[o_d + d][1][o_w * dw_tile_w + w + padding_left_or_right] =
								fms_zero_point;
						}
					}
				}
...
void dw_conv_copy_to_ifm_buffer(fms_dt ifms_buffer_lower_part[max_strides][max_strides][switch_point_fms_width + max_padding_lr],
...
#pragma HLS UNROLL
			...
			ifms_buffer[d][filter_dim - strides][w] = ifms_buffer_lower_part[d][0][w];
			if (strides == 2)
			{
				ifms_buffer[d][filter_dim - strides + 1][w] = ifms_buffer_lower_part[d][1][w];
			}
		}
	}
}
...
#pragma HLS UNROLL
					...
					if (c_h == 0 && c_w == 0)
					{
						result_tile[d][w] = ifms_buffer[d][c_h][w * strides + c_w + skip_padding_left] * weights[d][c_h * filter_dim + c_w];
					}
					else
					{
						result_tile[d][w] += ifms_buffer[d][c_h][w * strides + c_w + skip_padding_left] * weights[d][c_h * filter_dim + c_w];
...
void normalize_result_tile(dw_pss_dt result_tile[dw_pipeline_depth][switch_point_fms_width],
						   fms_dt normalized_result_tile[dw_pipeline_depth][switch_point_fms_width],
...
#pragma HLS PIPELINE
		...
		normalization.fused_scales_log_2_shift = fused_scales_log_2_shifts_tile[d];
		normalization.relu_6_fused_scale = relu_6_fused_scales_tile[d];
		normalization.fused_zero_point = fused_zero_points_tile[d];
		for (int w = 0; w < switch_point_fms_width; w++)
		{
#pragma HLS UNROLL
			if (w >= ifms_width)
			...
			normalized_result_tile[d][w] = dw_relu_norm(
				result_tile[d][w], normalization,
				layer_relu);
		}
	}
}

void write_back_result_tile(fms_dt result_tile[dw_pipeline_depth][switch_point_fms_width],
...
#pragma HLS UNROLL
					...
					result[current_absolute_offset_after_o_w + d * dw_tile_hw + w] = result_tile[d_in_pipeline * dw_tile_d + d][o_w * dw_tile_w + w];
				}
			}
		}
	}
...
#pragma HLS ARRAY_PARTITION variable = normalized_result_tile type = complete dim = 2
		 ...
		 dw_pipeline_in_d++)
	{
		...
		for (int h = 0; h < dw_num_of_runs_in_pipe + layer_ifm_height / strides; h++)
		{
			int read_results_h = h < layer_ifm_height / strides ? h : (layer_ifm_height / strides) - 1;
			const int absolute_offset_in_ifms = (tile_in_d * num_of_ifms_tiles_hw + ((read_results_h * strides + rows_filled_first_time) / dw_tile_h) *
																						num_of_ifms_tiles_w) *
													dw_tile_size +
												((read_results_h * strides + rows_filled_first_time) % dw_tile_h) * dw_tile_w;

			const int absolute_offset_in_ifms_2 = (tile_in_d * num_of_ifms_tiles_hw + ((read_results_h * strides + rows_filled_first_time + 1) / dw_tile_h) *
																						  num_of_ifms_tiles_w) *
													  dw_tile_size +
												  ((read_results_h * strides + rows_filled_first_time + 1) % dw_tile_h) * dw_tile_w;

			int write_to_results_h = (h - dw_num_of_runs_in_pipe) < 0 ? 0 : (h - dw_num_of_runs_in_pipe);
			const int absolute_offset_in_results = (tile_in_d * num_of_ofms_tiles_hw + (write_to_results_h / dw_tile_h) * num_of_ofms_tiles_w) *
													   dw_tile_size +
												   (write_to_results_h % dw_tile_h) * dw_tile_w;

			//*************************

			write_back_result_tile(normalized_result_tile, result,
			...
			normalize_result_tile(engine_result_tile, normalized_result_tile, normalization, 6, fused_scales_tile, fused_scales_log_2_shifts_tile,
								  relu_6_fused_scales_tile, fused_zero_points_tile, layer_ifm_width);
			...
			dw_conv_copy_to_ifm_buffer(ifms_buffer_lower_part,
									   ifms_buffer,
									   strides,
									   3,
									   layer_ifm_width,

...
#pragma HLS UNROLL
            ...
            {
                ifms_buffer[d_in_pipeline][h + offset_in_ifms_buffer_h][w + offset_in_ifms_buffer_w] = zero_point;
            }
        }
    }
}
...
#pragma HLS UNROLL
            ...
            {
                ifms_buffer[d_in_pipeline][offset_in_ifms_buffer_h + h][offset_in_ifms_buffer_w + w] = zero_point;
            }
        }
    }
}
...
#pragma HLS UNROLL
            ...
            if (h + starting_fill_h_offset < ifms_height && tile_index_in_h >= 0 && w + starting_fill_w_offset < ifms_width && tile_index_in_w >= 0)
            {
                ifms_buffer[d_in_pipeline][h + offset_in_ifms_buffer_h][w + offset_in_ifms_buffer_w] = channels[absolute_offset + h * (max_filter_hw_dim - 1) + w];
            }
            else
            {
                ifms_buffer[d_in_pipeline][h + offset_in_ifms_buffer_h][w + offset_in_ifms_buffer_w] = zero_point;
...
void fill_dw_tile(fms_dt channels[max_fms_size],
                  ...
                  int ifm_tile_in_w, int tile_offset_in_d,
                  const int num_of_tiles_h, const int num_of_tiles_w,
                  const int padding_left, const int padding_top, const int padding_right,
                  const int padding_bottom, fms_dt fms_zero_point)
{
#pragma HLS INLINE
...
void dw_conv_engine(
    ...
    fms_dt ifms_buffer[dw_pipeline_depth][dw_max_v2_buffer_height][dw_max_v2_buffer_width],
    dw_pss_dt result_tile[dw_pipeline_depth][dw_tile_h][dw_tile_w], const int filter_dim,
    const int strides)
{
#pragma HLS INLINE off
dw_conv_engine:
...
#pragma HLS UNROLL
                        if (c_h == 0 && c_w == 0)
                        {
                            result_tile[d_in_pipeline][h][w] = 0;
                        }
                        ...
                        result_tile[d_in_pipeline][h][w] += ifms_buffer[d_in_pipeline][h * strides + c_h][w * strides + c_w] * weights[d_in_pipeline][c_h * filter_dim + c_w];
                    }
                }
            }
        }
...
void normalize_and_write_back_result_tile(fms_dt result[max_fms_size],
                                          ...
                                          const int offset_h_in_tile,
                                          const int offset_w_in_tile, const int strides)
{
#pragma HLS INLINE
    ...
    for (int d_in_pipeline = 0; d_in_pipeline < dw_pipeline_depth; d_in_pipeline++)
    {
        ...
        normalization.fused_scales_log_2_shift = fused_scales_log_2_shifts_tile[d_in_pipeline];
        normalization.relu_6_fused_scale = relu_6_fused_scales_tile[d_in_pipeline];
        normalization.fused_zero_point = fused_zero_points_tile[d_in_pipeline];

        const int to_wrie_absolute_offset = absolute_offset_in_results + d_in_pipeline * num_of_tiles_hw * dw_tile_size +
                                            offset_h_in_tile * dw_tile_w + offset_w_in_tile;

    normalize_and_write_back_result_tile:
        for (int h = 0; h < dw_tile_h; h++)
        {
#pragma HLS PIPELINE
            for (int w = 0; w < dw_tile_w; w++)
...
#pragma HLS unroll
                ...
                result[to_wrie_absolute_offset + h * dw_tile_w + w] = dw_relu_norm(
                    result_tile[d_in_pipeline][h][w], normalization, layer_relu);
            }
        }
    }
}
             ...
             ofm_tile_in_h++)
        {
                 ...
                 ofm_tile_in_w++)
            {
                ...
                dw_innermost_loop:
                    ...
                    {
                        const int absolute_offset_in_ofms = (tile_in_d * num_of_ofms_tiles_hw + ofm_tile_in_h * num_of_ofms_tiles_w + ofm_tile_in_w) * dw_tile_size;
                        int ifm_tile_in_h = ofm_tile_in_h * strides + ifm_tile_in_ofm_tile_h;
                        const int ifm_tile_in_w = ofm_tile_in_w * strides + ifm_tile_in_ofm_tile_w;
                        const int absolute_offset_in_ifms = (tile_in_d * num_of_ifms_tiles_hw + ifm_tile_in_h * num_of_ifms_tiles_w + ifm_tile_in_w) * dw_tile_size;
                        ...
                        fill_dw_tile(channels, ifms_buffer, layer_ifm_width,
                                     ...
                                     num_of_ifms_tiles_h, num_of_ifms_tiles_w, padding_left, padding_top,

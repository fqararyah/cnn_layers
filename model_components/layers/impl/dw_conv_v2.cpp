...
#pragma HLS UNROLL
			left_col_segment[d][h] =
				ifms_buffer[d][h + use_upper_row][dw_max_v2_buffer_width - filter_dim + 1];
		}
	}
}

void padd_ifms_tile_depth_row(fms_dt row[dw_tile_d][max_dw_input_width],
							  const fms_dt zero_point, const int padding_left,
							  const int padding_right, const int layer_ifm_width)
{
...
#pragma HLS UNROLL
			upper_row[d][w + starting_shift_index] =
				ifms_buffer[d][dw_max_v2_buffer_height - filter_dim + 1][w + padding_left];
		}
	}
}
...
#pragma HLS INLINE
	...
	const int last_row_to_fill = (dw_max_v2_buffer_height - 1) - 1 + use_upper_row;
	const int last_col_to_fill = (dw_max_v2_buffer_width - 1) - 1 + use_left_col;
	// fill upper row
	if (use_upper_row)
	{
		for (int d = 0; d < dw_tile_d; d++)
...
void dw_conv_engine(dw_weights_dt weights_tile[dw_tile_d][3 * 3],
					fms_dt ifms_buffer[dw_tile_d][dw_max_v2_buffer_height][dw_max_v2_buffer_width],
					...
					biases_dt fused_zero_points_tile[])
{
#pragma HLS INLINE

	const int ifm_tile_offset_in_ofm_tile_h = ifm_tile_in_ofm_tile_h * (dw_tile_h / strides);
	const int ifm_tile_offset_in_ofm_tile_w = ifm_tile_in_ofm_tile_w * (dw_tile_w / strides);
	const int start_writing_index = absolute_offset_in_ofms + ifm_tile_offset_in_ofm_tile_h * dw_tile_w + w_offset_in_tile + ifm_tile_offset_in_ofm_tile_w;
...
#pragma HLS UNROLL
				...
				{
#pragma HLS UNROLL
					tmp_pss += weights_tile[d][c_h * filter_dim + c_w] * ifms_buffer[d][h * strides + c_h][w_offset_in_tile * strides + c_w];
					//					if (absolute_offset_in_ofms == 0 && d == 0) {
...
void dw_conv_pipeline(fms_dt channels[max_fms_size],
					  ...
					  const int layer_relu, fused_scales_dt fused_scales_tile[],
					  fused_scales_log_2_shifts_dt fused_scales_log_2_shifts_tile[],
					  relu_6_fused_scales_dt relu_6_fused_scales_tile[],
					  biases_dt fused_zero_points_tile[])
{
#pragma HLS INLINE
	...
	if ((ifm_tile_in_w + 1) < num_of_ifms_tiles_w)
	{
		fill_ifms_tile_depth_row_segment(channels, lower_row, ifm_tile_in_w + 1,
										 absolute_offset_in_ifms + (num_of_ifms_tiles_w + 1) * dw_tile_size,
										 padding_left, padding_right, is_padding_row, ifm_width,
										 fms_zero_point);
	}
	...
	{
#pragma HLS PIPELINE
		dw_conv_engine(weights_tile, ifms_buffer, result, w, tile_d_offset,
					   absolute_offset_in_ofms, ifm_tile_in_ofm_tile_h,
					   ifm_tile_in_ofm_tile_w, filter_dim, strides, normalization,
					   layer_relu, fused_scales_tile, fused_scales_log_2_shifts_tile,
...
#pragma HLS ARRAY_PARTITION variable = weights_tile type = complete dim = 0
	...
	padd_ifms_tile_depth_row(upper_row, current_layer_fms_zero_point,
							 padding_left, padding_right, layer_ifm_width);
	padd_ifms_tile_depth_row(lower_row, current_layer_fms_zero_point,
							 padding_left, padding_right, layer_ifm_width);
	...
	for (int tile_in_d = 0; tile_in_d < num_of_tiles_d; tile_in_d++)
	{
			 ...
			 ofm_tile_in_h++)
		{
				 ...
				 ifm_tile_in_ofm_tile_h++)
			{
				...
				int absolute_offset_in_ifms = tile_in_d * num_of_ifms_tiles_hw * dw_tile_size + (ofm_tile_in_h * strides + ifm_tile_in_ofm_tile_h) * num_of_ifms_tiles_w * dw_tile_size;
...
#pragma HLS ARRAY_PARTITION variable = ifms_buffer type = complete dim = 0
						...
						dw_conv_pipeline(channels, weights_tile, result,
										 upper_row, lower_row, ifms_buffer,
										 col_segment_left, col_segment_right, 3, strides,
										 padding_left, padding_right, padding_top,
										 layer_ifm_width, layer_ifm_height,
										 ifm_tile_in_h, ifm_tile_in_w,
										 tile_in_d * dw_tile_d, num_of_ifms_tiles_w,
										 num_of_ifms_tiles_h, absolute_offset_in_ifms,
										 absolute_offset_in_ofms + (ofm_tile_in_w * dw_tile_size),

...
void fill_ifms_tile_depth_col_segment(fms_dt channels[max_fms_size],
		const int tile_index_in_h, fms_dt col_segment[dw_tile_d][dw_tile_h],
...
void fill_ifms_tile_depth_row_segment(fms_dt channels[max_fms_size],
		fms_dt row[dw_tile_d][max_dw_input_width], const int tile_index_in_w, int absolute_offset,
		const int padding_left, const bool is_padding_row, const int ifms_width,
		const fms_dt zero_point) {
#pragma HLS INLINE
...
#pragma HLS INLINE
	...
	const int starting_shift_index = tile_index_in_w * dw_tile_size
			+ padding_left;
	for (int w = 0; w < dw_tile_w; w++) {
#pragma HLS UNROLL
		for (int d = 0; d < dw_tile_d; d++) {
#pragma HLS UNROLL
			upper_row[d][w + starting_shift_index] = lower_row[d][w
					+ starting_shift_index];
		}
	}
}
...
#pragma HLS UNROLL
		for (int d = 0; d < dw_tile_d; d++) {
#pragma HLS UNROLL
			ifms_row[d][w + ifms_width] = zero_point;
		}
	}
}
...
void dw_fill_ifms_buffer(fms_dt channels[max_fms_size],
		...
		fms_dt upper_row[dw_tile_d][max_dw_input_width], fms_dt lower_row[dw_tile_d][max_dw_input_width],
...
#pragma HLS UNROLL
			ifms_buffer[d][h + use_upper_row][dw_max_v2_buffer_width
					- use_left_col - 1] = right_col_segment[d][h];
		}
	}
...
void dw_conv_engine(const dw_weights_dt weights[max_conv_d][max_conv_h * max_conv_w],
		fms_dt ifms_buffer[dw_tile_d][dw_max_v2_buffer_height][dw_max_v2_buffer_width],
		fms_dt result[max_fms_size], int w_offset_in_tile,
		const int tile_d_offset, int absolute_offset_in_ifms,
...
#pragma HLS INLINE
	...
	const int ifm_tile_offset_in_ofm_tile_h = ifm_tile_in_ofm_tile_h
			* (dw_tile_h / strides);
	const int ifm_tile_offset_in_ofm_tile_w = ifm_tile_in_ofm_tile_h
			* (dw_tile_w / strides);
	for (int d = 0; d < dw_tile_d; d++) {
...
#pragma HLS UNROLL
					tmp_pss += weights[tile_d_offset][c_h * filter_dim + c_w]
							* ifms_buffer[d][h * strides + c_h][w_offset_in_tile
									* strides + c_w];
				}
			}
			result[absolute_offset_in_ifms + d * dw_tile_hw
					+ (ifm_tile_offset_in_ofm_tile_h + h) * dw_tile_w
					+ ifm_tile_offset_in_ofm_tile_w + w_offset_in_tile] =
...
void dw_conv_pipeline(fms_dt channels[max_fms_size],
		const dw_weights_dt weights[max_conv_d][max_conv_h * max_conv_w], fms_dt result[max_fms_size],
		...
		int absolute_offset_in_ifms, int ifm_tile_in_ofm_tile_h,
...
#pragma HLS INLINE off
	...
	dw_fill_ifms_buffer(channels, ifms_buffer, upper_row, lower_row,
			...
			ifm_height, ifm_width, num_of_ifms_tiles_w, num_of_ifms_tiles_h,
			strides == 1, strides == 1);

	for (int w = 0; w < dw_tile_w / strides; w++) {
#pragma HLS PIPELINE
		dw_conv_engine(weights, ifms_buffer, result, w, tile_d_offset,
				absolute_offset_in_ifms, ifm_tile_in_ofm_tile_h,
				ifm_tile_in_ofm_tile_w, filter_dim, strides, normalization,
				layer_relu);
	}

	bool is_padding_row = (padding_top && ifm_tile_in_h == 0)
			|| ifm_tile_in_h == num_of_ifms_tiles_h - 1;
...
#pragma HLS INLINE off
	...
	for (int tile_in_d = 0; tile_in_d < num_of_tiles_d; tile_in_d++) {
		absolute_index_in_ifms = tile_in_d * num_of_ifms_tiles_hw
				* dw_tile_size;
		absolute_index_in_ofms = tile_in_d * num_of_ofms_tiles_hw
				* dw_tile_size;
		...
		for (int ofm_tile_in_h = 0; ofm_tile_in_h < num_of_ofms_tiles_h;
			...
			for (int ifm_tile_in_ofm_tile_h = 0;
				...
				fill_ifms_tile_depth_col_segment(channels, 0, col_segment_left,
						absolute_index_in_ifms, true, layer_ifm_height,
						current_layer_fms_zero_point);

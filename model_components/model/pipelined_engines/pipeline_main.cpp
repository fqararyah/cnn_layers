    ...
    for (int d = 0; d < layer_depth; d++)
    {
        for (int h = 0; h < PIPE_TO_SEML_NUM_ROWS_TO_COPY; h++)
        {
            ...
            for (int w = 0; w < layer_ifms_width; w++)
            {
                ...
                result[tile_index][h_in_tile][w_in_tile] = pipe_seml_communication_buffer[d][h + offset_h_in_communication_buffer][w];
            }
        }
    }
}
...
void pipelined_engines_caller(fms_dt result[MAX_FMS_BUFFER_DEPTH][MIN_FMS_HEIGHT][MIN_FMS_WIDTH])
{
    ...
    for (int h = 0; h < layer_10_pw_specs.layer_ifm_height; h += pipe_rows_produced_in_a_pass)
    {
        for (int i = 0; i < 2; i++)
        { // todo change 2
            fill_pipe_layer_input_buffer(
        ...
        pw_dw_conv(on_chip_pw_weights,
                   ...
                   dw_channels_tile,
                   dw_channels_tile_copy,
                   h + rows_produced_in_pipeline_filling_phase + 1, // starting_h
                   0,                                               // h_offset_in_result,
                   0,
                   layer_7_pw_specs,
                   layer_6_dw_specs,
        ...
        write_pipe_seml_communication_buffer(
            channels_buffer,
            result,
            h + rows_produced_in_pipeline_filling_phase, // starting_h
            0,
            first_layer_in_second_part);
    }
}

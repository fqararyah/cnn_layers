...
#pragma HLS INLINE off
    ...
    const int filling_weights_offset =
        (layer_specs_struct.layer_weights_offset_on_chip + starting_filter * layer_depth) /
    ...
    for (int f = 0; f < PARALLELISM_PW_OFMS; f++)
    {
        ...
        for (int h = 0; h < PW_BUFFER_HEIGHT; h++)
        {
            ...
            int current_read_offset_in_overlap_buffer =
                d_offset_in_overlap_buffer + f * filter_dim_minus_strides * num_of_tiles_in_w + (h_in_overlap_buffer * layer_ifms_width) / DW_PIPE_OVERLAP_BUFFER_WIDTH;
            ...
            for (int w = 0; w < MAX_FILTER_MINUS_STRIDES; w++)
            {
                ...
                if (h_in_overlap_buffer >= 0)
                {
                    normalized_tile[f][h_in_overlap_buffer][w] =
                        dw_pipe_overlap_buffer[current_read_offset_in_overlap_buffer][w - padding_left];
...
#pragma HLS UNROLL
                    ...
                    if (fused_pw_dw)
                    {
                        if (starting_h + h < layer_ifms_height && (DW_BUFFER_HEIGHT - filter_dim != h + write_offset_h_in_normalized_tile || padding_top == 0 || starting_h != 0) && starting_w + filter_minus_strides + w < layer_ifms_width + padding_left)
                        {
                            normalized_tile[f][h + write_offset_h_in_normalized_tile][filter_minus_strides + w] = pw_relu_norm_6(tmp_pss,
                                                                                                                                 current_normaliztion_scheme, layer_relu);
                        }
                        else
                        {
                            normalized_tile[f][h + write_offset_h_in_normalized_tile][filter_minus_strides + w] = dw_layer_ifms_zero_point;
                        }
                    }
                    else
                    {
...
#pragma HLS UNROLL
                    if (w + starting_w + columns_filled_by_pw_in_the_firts_pass < layer_ifms_width)
                    {
                        if (w + inner_offset_w + columns_filled_by_pw_in_the_firts_pass >= DW_PIPE_OVERLAP_BUFFER_WIDTH)
                        {
                            ...
                            {
                                read_val =
                                    dw_pipe_overlap_buffer[current_read_offset_in_overlap_buffer + 1][w + inner_offset_w + columns_filled_by_pw_in_the_firts_pass - DW_PIPE_OVERLAP_BUFFER_WIDTH];
                            }
                            dw_pipe_overlap_buffer[current_write_offset_in_overlap_buffer + 1][w + inner_offset_w + columns_filled_by_pw_in_the_firts_pass - DW_PIPE_OVERLAP_BUFFER_WIDTH] =
                        ...
                        else
                        {
                            ...
                            {
                                read_val =
                                    dw_pipe_overlap_buffer[current_read_offset_in_overlap_buffer][w + inner_offset_w + columns_filled_by_pw_in_the_firts_pass];
                            }
                            dw_pipe_overlap_buffer[current_write_offset_in_overlap_buffer][w + inner_offset_w + columns_filled_by_pw_in_the_firts_pass] =
                                dw_channels_tile[d][write_offset_h_in_channels_tile + h][w + filter_minus_strides];
                        }
                    }
                    else
                    ...
                    {
                        dw_vertical_overlap_buffer[d][h][w + filter_minus_strides - PARALLELISM_PW_W] =
                            read_val;
                    }
                }
            }

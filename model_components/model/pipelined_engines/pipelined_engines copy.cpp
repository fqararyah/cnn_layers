...
#pragma HLS UNROLL
        ...
        {
#pragma HLS UNROLL
            conv_dw_communication_buffer_intra_w[d][h][w] =
                conv_dw_communication_buffer_intra_r[d][h][w + strides];
        }
        ...
        if (f > 0)
        {
            if (!direction)
            {
                shift_and_fill_slice_of_conv_dw_communication_buffer_intra(
                    ...
                    conv_dw_communication_buffer_intra_r,
                    conv_dw_communication_buffer_intra_w,
                    f - 1, conv_dw_comm_buffer_reading_row, starting_w);
            }
            else
            {
                shift_and_fill_slice_of_conv_dw_communication_buffer_intra(
        ...
        if (dw_starting_h >= 0 && dw_starting_w >= 0)
        {
            if (!direction)
            {
                pre_first_pipeline_layers_output[f][dw_starting_h][dw_starting_w] =
                    first_dw_layer_kernel(conv_dw_communication_buffer_intra_r,
            ...
            {
                pre_first_pipeline_layers_output[f][dw_starting_h][dw_starting_w] =
                    first_dw_layer_kernel(conv_dw_communication_buffer_intra_w,
                                          dw_layer_weights, layer_2_dw_filter_dim, f,
                                          first_dw_layer_quantization_params[f],
                                          layer_2_dw_specs.layer_activation);
            }

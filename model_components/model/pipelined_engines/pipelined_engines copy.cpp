...
#pragma HLS ARRAY_PARTITION variable = pw_engine_result_tile_copy type = complete dim = 0
    ...
    fms_dt dw_vertical_overlap_buffer[DW_TILE_DEPTH][DW_BUFFER_HEIGHT][MAX_FILTER_MINUS_STRIDES];
    fms_dt dw_vertical_overlap_buffer_copy[DW_TILE_DEPTH][DW_BUFFER_HEIGHT][MAX_FILTER_MINUS_STRIDES];
...
#pragma HLS ARRAY_PARTITION variable = dw_vertical_overlap_buffer_copy type = complete dim = 2
#pragma HLS ARRAY_PARTITION variable = dw_vertical_overlap_buffer_copy type = complete dim = 3

    fms_dt dw_horizontal_overlap_buffer[DW_TILE_DEPTH][MAX_FILTER_MINUS_STRIDES][PW_BUFFER_WIDTH];
    fms_dt dw_horizontal_overlap_buffer_copy[DW_TILE_DEPTH][MAX_FILTER_MINUS_STRIDES][PW_BUFFER_WIDTH];
...
#pragma HLS ARRAY_PARTITION variable = dw_horizontal_overlap_buffer_copy type = complete dim = 2
#pragma HLS ARRAY_PARTITION variable = dw_horizontal_overlap_buffer_copy type = complete dim = 3
...
#pragma HLS ARRAY_PARTITION variable = pw_engine_input_buffer_copy type = complete dim = 3
        ...
        for (int d = 0; d < num_of_filters; d += PARALLELISM_PW_OFMS)
        {
            ...
            for (int w = 0; w < ifms_width; w += PW_BUFFER_WIDTH)
            {
                ...
                if ((w / PW_BUFFER_WIDTH) % 2 == 0)
                {
                    ...
                    write_next_overlap_and_read_current_only_p2(
                        ...
                        dw_channels_tile, d, starting_h, prev_prev_w,
                    ...
                    pw_normalize_engine_result(pw_engine_result_tile_copy,
                                               dw_channels_tile_copy, dw_vertical_overlap_buffer_copy,
                                               dw_horizontal_overlap_buffer_copy,
                ...
                else
                {
                    ...
                    //###############################
                    write_next_overlap_and_read_current_only_p2(
                        dw_pipe_overlap_buffer, dw_vertical_overlap_buffer_copy,
                        dw_horizontal_overlap_buffer_copy,
                        dw_channels_tile_copy, d, starting_h, prev_prev_w,
                        dw_layer_specs_struct, odd_even);
            ...
            if (((ifms_width / PW_BUFFER_WIDTH) - 1) % 2)
            {
                ...
                write_next_overlap_and_read_current_only_p2(
                        dw_pipe_overlap_buffer, dw_vertical_overlap_buffer,
                        dw_horizontal_overlap_buffer,
                        dw_channels_tile, d, starting_h, ifms_width - 2 * PW_BUFFER_WIDTH,
                        dw_layer_specs_struct, odd_even);
                dw_conv_engine(dw_weights_tile, dw_channels_tile,
                               dw_result_tile, dw_layer_specs_struct);
                //###############################
                pw_normalize_engine_result(pw_engine_result_tile_copy,
                                           ...
                                           dw_horizontal_overlap_buffer_copy,
                                           pw_normalization_buffer_copy, d, starting_h,
                ...
                write_next_overlap_and_read_current_only_p2(
                        dw_pipe_overlap_buffer, dw_vertical_overlap_buffer_copy,
                        dw_horizontal_overlap_buffer_copy,
                        dw_channels_tile_copy, d, starting_h, ifms_width - PW_BUFFER_WIDTH,
                        dw_layer_specs_struct, odd_even);
                dw_conv_engine(dw_weights_tile, dw_channels_tile_copy,
                               dw_result_tile, dw_layer_specs_struct);
                //#######################################################################################
            ...
            else
            {
                ...
                write_next_overlap_and_read_current_only_p2(
                        dw_pipe_overlap_buffer, dw_vertical_overlap_buffer_copy,
                        dw_horizontal_overlap_buffer_copy,
                        dw_channels_tile_copy, d, starting_h, ifms_width - 2 * PW_BUFFER_WIDTH,
                        dw_layer_specs_struct, odd_even);
                dw_conv_engine(dw_weights_tile, dw_channels_tile_copy,
                               dw_result_tile_copy, dw_layer_specs_struct);
                //###############################
                ...
                write_next_overlap_and_read_current_only_p2(
                        dw_pipe_overlap_buffer, dw_vertical_overlap_buffer,
                        dw_horizontal_overlap_buffer,
                        dw_channels_tile, d, starting_h, ifms_width - PW_BUFFER_WIDTH,
                        dw_layer_specs_struct, odd_even);
                dw_conv_engine(dw_weights_tile, dw_channels_tile,
                               dw_result_tile, dw_layer_specs_struct);
                //#######################################################################################
    ...
    else
    {
        for (int d = 0; d < num_of_filters; d += PARALLELISM_PW_OFMS)
        {
            ...
            for (int w = 0; w < ifms_width; w += PW_BUFFER_WIDTH)
            {
                ...
                if ((w / PW_BUFFER_WIDTH) % 2 == 0)
                {
                    pw_write_back_result_tile(dw_channels_tile, result,
                                              tmp_channels, d, prev_prev_w, pw_layer_specs_struct);
                    //###############################
            ...
            {
                pw_write_back_result_tile(dw_channels_tile, result,
                                          tmp_channels, d, ifms_width - 2 * PW_BUFFER_WIDTH, pw_layer_specs_struct);
                //###############################
                pw_only_normalize_engine_result(pw_engine_result_tile_copy,
                                                dw_channels_tile_copy,
                                                tmp_channels, pw_normalization_buffer, d, starting_h,

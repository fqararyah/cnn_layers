...
void bottleneck_1_fill_projection_kernel_weights(
	...
	bottleneck_1_fill_projection_kernel_weights: for (int filter_index = 0;
...
void bottleneck_1_padding_top_right(
		...
		fms_dt zero_point) {
	bottleneck_1_padding_top_right: for (int d = 0;
...
void bottleneck_1_do_padding_left(
		...
		fms_dt zero_point) {
	bottleneck_1_do_padding_left: for (int d = 0;
			d < bottleneck_1_expanded_ifms_depth; d++) {
...
void bottleneck_1_update_previous_pass_buffer(
		fms_dt previous_pass_dw_input[][bottlenck_0_inter_pass_dw_input_height][bottlenck_0_inter_pass_dw_input_width],
		fms_dt dw_lower_buffer[][bottleneck_1_dw_filter_dim], int offset_w,
...
void mob_v2_bottleneck_1(fms_dt bottleneck_input[], fms_dt bottleneck_output[],
		fms_dt previous_pass_dw_input[][bottlenck_0_inter_pass_dw_input_height][bottlenck_0_inter_pass_dw_input_width],
		fms_dt dw_lower_buffer[][bottleneck_1_dw_filter_dim], int starting_h,
		int starting_w) {
#pragma HLS INLINE off
	...
	const int current_dw_starting_h = starting_h
			- (bottleneck_1_dw_filter_dim - bottleneck_1_dw_padding_top) + 1; //+1 is the current
	const int current_dw_starting_w = starting_w
			- (bottleneck_1_dw_filter_dim - bottleneck_1_dw_padding_left) + 1;
	const int non_first_step_in_w = starting_w != 0;
	shift_dw_ifms_buffer_horizontally_3x3_s1(dw_lower_buffer,
			bottleneck_1_dw_strides, bottleneck_1_expanded_ifms_depth);

	mob_v2_bottleneck_1: for (int d_in_out = 0;
...
#pragma HLS PIPELINE
		...
		fms_dt dw_input_buffer[bottleneck_1_dw_filter_dim
				* bottleneck_1_dw_filter_dim];
#pragma HLS ARRAY_PARTITION variable = dw_input_buffer type = complete dim = 0

		if (starting_h < bottleneck_1_ifms_height + bottleneck_1_dw_padding_top
				&& starting_w < bottleneck_1_ifms_width / layer_0_s_strides) {
		...
		if (starting_w
				>= bottleneck_1_dw_filter_dim - bottleneck_1_dw_padding_left and d_in_out > 0) {
			bottleneck_1_update_previous_pass_buffer(previous_pass_dw_input,
					dw_lower_buffer, starting_w - 1, starting_h, d_in_out - 1);
		}
		bottleneck_1_fill_projection_kernel_weights(pw_weights_2,
	...
	normalize_projection_kernel_output(projection_results_buffer,
			...
			bottleneck_1_ofms_depth, layer_2_activation,
			bottleneck_1_projection_layer_index);
	if (starting_w
			>= bottleneck_1_dw_filter_dim - bottleneck_1_dw_padding_left) {
		bottleneck_1_update_previous_pass_buffer(previous_pass_dw_input,
				dw_lower_buffer, starting_w - 1, starting_h, bottleneck_1_expanded_ifms_depth - 1);
	}
}

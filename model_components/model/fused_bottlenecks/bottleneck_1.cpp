...
void bottleneck_0_fill_projection_kernel_weights(const weights_dt layer_weights[][bottleneck_0_expanded_ifms_depth],
												 weights_dt kernel_weights[], int d)
{
	for (int filter_index = 0; filter_index < bottleneck_0_ofms_depth; filter_index++)
	{
									...
									fms_dt zero_point)
{
	for (int d = 0; d < bottleneck_0_expanded_ifms_depth; d++)
	{
		for (int h = 0; h < bottleneck_0_dw_padding_top; h++)
		{
			...
			for (int w = bottleneck_0_ifms_width; w < bottlenck_0_inter_pass_dw_input_width; w++)
			{
				previous_pass_dw_input[d][h][w] = zero_point;
			}
		}
								  ...
								  fms_dt dw_lower_buffer[][bottleneck_0_dw_filter_dim], fms_dt zero_point)
{
	for (int d = 0; d < bottleneck_0_expanded_ifms_depth; d++)
	{
		...
		dw_lower_buffer[d][bottleneck_0_dw_filter_dim - bottleneck_0_dw_strides] = zero_point;
	}
}

void bottleneck_0_update_previous_pass_buffer(fms_dt previous_pass_dw_input[][bottlenck_0_inter_pass_dw_input_height][bottlenck_0_inter_pass_dw_input_width],
											  fms_dt dw_lower_buffer[][bottleneck_0_dw_filter_dim], int offset_w, int starting_h)
{
	for (int d = 0; d < bottleneck_0_expanded_ifms_depth; d++)
	{
		if (starting_h != 0)
		{
			previous_pass_dw_input[d][0][offset_w] = previous_pass_dw_input[d][1][offset_w];
		}
		previous_pass_dw_input[d][1][offset_w] = dw_lower_buffer[d][0];
		if (offset_w + 1 == bottleneck_0_ifms_width / layer_0_s_strides)
		{
...
void bottleneck_0_fill_dw_input(fms_dt previous_pass_dw_input[][bottlenck_0_inter_pass_dw_input_height][bottlenck_0_inter_pass_dw_input_width],
								fms_dt dw_lower_buffer[][bottleneck_0_dw_filter_dim],
								fms_dt dw_input_buffer[],
								fms_dt expansion_layer_result,
								int filling_d, int filling_w_offset, int absolute_starting_h)
{

	if (filling_w_offset > 0 && absolute_starting_h != 0)
	{
		dw_input_buffer[0] = previous_pass_dw_input[filling_d][0][filling_w_offset - 1];
		dw_input_buffer[1] = previous_pass_dw_input[filling_d][0][filling_w_offset];
		dw_input_buffer[2] = previous_pass_dw_input[filling_d][0][filling_w_offset + 1];
		dw_input_buffer[3] = previous_pass_dw_input[filling_d][1][filling_w_offset - 1];
		dw_input_buffer[4] = previous_pass_dw_input[filling_d][1][filling_w_offset];
		dw_input_buffer[5] = previous_pass_dw_input[filling_d][1][filling_w_offset + 1];
...
#pragma HLS INLINE off
	...
	const int current_dw_starting_h = starting_h - (bottleneck_0_dw_filter_dim - bottleneck_0_dw_padding_top) + 1; //+1 is the current
	const int current_dw_starting_w = starting_w - (bottleneck_0_dw_filter_dim - bottleneck_0_dw_padding_left) + 1;
	const int non_first_step_in_w = starting_w != 0;
	shift_dw_ifms_buffer_horizontally_3x3_s1(dw_lower_buffer, bottleneck_0_dw_strides,
...
#pragma HLS PIPELINE
		...
		expansion_layer_normalization.layer_0_relu_6_fused_scale = layer_0_s_relu_6_fused_scales[d_in_out];
...
#pragma HLS ARRAY_PARTITION variable = dw_input_buffer type = complete dim = 0
		...
		if (starting_h < bottleneck_0_ifms_height + bottleneck_0_dw_padding_top && starting_w < bottleneck_0_ifms_width / layer_0_s_strides)
		{
		...
		bottleneck_0_fill_dw_input(previous_pass_dw_input, dw_lower_buffer, dw_input_buffer, expansion_result,
								   d_in_out, starting_w, starting_h);
		...
		dw_pss_dt dw_pss = dw_kernel(dw_input_buffer, dw_weights_1, bottleneck_0_dw_filter_dim,
									 d_in_out);
		fms_dt dw_result = dw_relu_norm(dw_pss, dw_layer_normalization,
										bottleneck_0_dw_layer_relu);

		bottleneck_0_fill_projection_kernel_weights(pw_weights_2, projection_kernel_weights, d_in_out);
	...
	normalize_projection_kernel_output(projection_results_buffer,
									   ...
									   layer_2_pw_fused_zero_points, bottleneck_0_ofms_depth,
									   layer_2_activation, bottleneck_0_projection_layer_index);
	if (starting_w >= bottleneck_0_dw_filter_dim - bottleneck_0_dw_padding_left)
	{
		bottleneck_0_update_previous_pass_buffer(previous_pass_dw_input, dw_lower_buffer, starting_w - 1, starting_h);
	}
}

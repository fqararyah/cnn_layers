...
void mob_v2_bottleneck_0(fms_dt bottleneck_input[],
						 ...
						 fms_dt previous_pass_dw_input[][bottleneck_0_inter_pass_dw_input_height][bottleneck_0_inter_pass_dw_input_width],
						 fms_dt dw_lower_buffer[][bottleneck_0_dw_filter_dim], const int starting_h, const int h_in_communication_buffer,
...
#pragma HLS INLINE off
	...
	shift_dw_ifms_buffer_horizontally_3x3_s1(dw_lower_buffer, bottleneck_0_expanded_ifms_depth);
...
#pragma HLS ARRAY_PARTITION variable = dw_input_buffer type = complete dim = 0
		...
		if (starting_w >= bottleneck_0_dw_filter_dim - bottleneck_0_dw_padding_left and d_in_out > 0)
		{
		...
		if (starting_w > 1 && d_in_out < bottleneck_0_ofms_depth)
		{
			// if(starting_h == 1 && d_in_out == 15)cout<<starting_w - 2<<"\n";
			bottleneck_0_1_communication_buffer[d_in_out][h_in_communication_buffer][starting_w - 2] =
	...
	if (starting_w > 0)
	{
		copy_projection_kernel_output_buffer(projection_kernel_output_buffer, projection_kernel_output_buffer_prev, bottleneck_0_ofms_depth);
	}

...
#pragma HLS INLINE
	...
	pss_dt pss = 0;
	const int filling_offset = filling_h * bottleneck_ifms_depth
			* bottleneck_ifms_width + filling_w * bottleneck_ifms_depth;
...
#pragma HLS UNROLL
			ifms_buffer[h * filter_dim + w] = ifms_buffer[h * filter_dim + w
					+ strides];
		}
	}
}
...
#pragma HLS UNROLL
			ifms_buffer[h * filter_dim + w] = filling_src[h * ifms_dw
					+ (ifms_w_offset + w - start_filling_offset_w) * ifms_depth
					+ filling_d];
		}
	}
}
...
#pragma HLS INLINE
	...
	const int rows_to_shift =
			(filter_dim - strides) - strides < 0 ?
					0 : (filter_dim - strides) - strides;
	const int ifms_dw = ifms_depth * ifms_width;
	const int end_filling_offset_h = filter_dim - strides;
...
#pragma HLS UNROLL
			ifms_buffer[h * filter_dim + w] = filling_src[(h
					- start_filling_offset) * strides
					+ (w - start_filling_offset)];
		}
	}
}
...
#pragma HLS INLINE
	...
	for (int c_h = 0; c_h < filter_dim; c_h++) {
		for (int c_w = 0; c_w < filter_dim; c_w++) {
			pss += ifms_buffer[c_h * filter_dim + c_w]
					* weights[conv_d][c_h * filter_dim + c_w];
		}
	}
	return pss;
}

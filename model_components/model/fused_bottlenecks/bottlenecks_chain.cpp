...
void bottleneck_chain_0_1_fill_ifm_groups_buffer(
		...
		fms_grp_dt fms_groups_buffer[input_image_depth][input_image_num_fms_groups_in_width
				* bottleneck_0_fill_each_time], int starting_h,
...
#pragma HLS UNROLL
				channels_buffer_0[d][h][w] = channels_buffer_0[d][h
						+ chain_0_1_rows_filled_each_time][w];
			}
		}
	}
}
...
#pragma HLS INLINE off
	...
	const int rows_to_shift = chain_0_1_in_buffer_height
			- chain_0_1_rows_filled_each_time;
	if (shift) {
...
#pragma HLS UNROLL
				if (starting_w + w
						< chain_0_1_ifms_width + chain_0_1_first_padding_left) {
					bottleneck_0_input[d * bottleneck_0_input_buffer_hw
							+ h * bottlenck_0_input_buffer_width + w
							+ start_filling_index_in_first_bottleneck_input] =
...
#pragma HLS PIPELINE
				bottleneck_1_input_buffer[h
						* chain_0_1_bottleneck_0_rows_at_once
						* bottleneck_1_ifms_depth + w * bottleneck_1_ifms_depth
						+ d] = bottleneck_0_1_communication_buffer_prev[d][h][w
						+ offset_w];
			}
		}
	}
...
#pragma HLS INLINE off
	...
	save_chain_output: for (int d = 0; d < chain_ofms_depth; d++) {
				...
				for (int w = 0; w < input_image_width; w++) {
					channels_buffer_0[d][h + channels_buffer_start_filling_h][w] =
							channels[d * input_image_hw
									+ (h + starting_h) * input_image_width + w];
				}
			}
		} else {
			chain_0_1_padd_bottom_channels_buffer_rows(channels_buffer_0,
	...
	for (int h = 0; h < chain_0_1_bottleneck_0_rows_at_once; h++) {
		...
		for (int o_w = 0; o_w < chain_0_1_bottleneck_0_rows_at_once; o_w++) {
			for (int i_w = 0; i_w < chain_0_1_ofms_width; i_w++) {
				...
				const int fill_input_index = (w + 1) * chain_0_1_first_strides
						+ bottleneck_0_first_fill_offset;
		...
		for (int d = 0; d < bottleneck_0_ofms_depth; d++) {
			bottleneck_0_1_communication_buffer[d][h][bottleneck_0_ofms_width
					- 1] = normalize_projection_kernel_output(
					bottleneck_0_projection_kernel_output_prev,
		...
		for (int d = 0; d < bottleneck_0_expanded_ifms_depth; d++) {
			if (h == 1) {
				bottleneck_0_previous_pass_dw_input[d][0][bottleneck_0_inter_pass_dw_input_width
						- bottleneck_0_dw_padding_right - 1] =
						bottleneck_0_previous_pass_dw_input[d][1][bottleneck_0_inter_pass_dw_input_width
								- bottleneck_0_dw_padding_right - 1];
			}
			bottleneck_0_previous_pass_dw_input[d][1][bottleneck_0_inter_pass_dw_input_width
					- bottleneck_0_dw_padding_right - 1] =
...
void bottleneck_1_within_pipeline_stage(
		...
		fms_dt bottleneck_1_dw_lower_buffer[bottleneck_1_expanded_ifms_depth][bottleneck_1_dw_filter_dim
...
#pragma HLS INLINE off
	...
	const int bottleneck_1_warm_up_rows = bottleneck_1_dw_filter_dim
			- bottleneck_1_dw_strides;
	int bottleneck_1_h =
			starting_h == 0 ?
					0 :
					starting_h * bottleneck_1_rows_at_once
							* bottleneck_1_dw_strides
							- bottleneck_1_warm_up_rows;
	...
	for (int o_w = 0; o_w < chain_0_1_bottleneck_1_rows_at_once; o_w++) {
		for (int i_w = 0; i_w < chain_0_1_ofms_width; i_w++) {
			...
			const int fill_input_index = w * bottleneck_1_dw_strides
					+ bottleneck_1_first_fill_offset;
			fill_bottleneck_1_input(bottleneck_0_1_communication_buffer,
...
#pragma HLS INLINE off
	...
	for (int d = 0;
			d
					< bottleneck_0_ofms_depth
							/ bottleneck_0_1_communication_buffer_partitioning_factor_in_d;
			d++) {
		for (int w = 0; w < bottleneck_0_ofms_width; w++) {
#pragma HLS PIPELINE
			for (int h = 0; h < chain_0_1_bottleneck_0_rows_at_once; h++) {
#pragma HLS UNROLL
				for (int d_inner = 0;
						...
						d_inner++) {
#pragma HLS UNROLL
					bottleneck_0_1_communication_buffer_prev[d
							* bottleneck_0_1_communication_buffer_partitioning_factor_in_d
							+ d_inner][h][w] =
							bottleneck_0_1_communication_buffer[d
									* bottleneck_0_1_communication_buffer_partitioning_factor_in_d
									+ d_inner][h][w];
				}
			}
...
#pragma HLS ARRAY_PARTITION variable = bottleneck_0_projection_kernel_output complete
	...
	fms_dt bottleneck_0_expansion_results_buffer[bottleneck_0_dw_filter_dim
			- bottleneck_0_dw_strides];
	fms_dt bottleneck_0_dw_lower_buffer[bottleneck_0_expanded_ifms_depth][bottleneck_0_dw_filter_dim];
	fms_dt bottleneck_0_previous_pass_dw_input[bottleneck_0_expanded_ifms_depth][bottleneck_0_inter_pass_dw_input_height][bottleneck_0_inter_pass_dw_input_width];
...
#pragma HLS ARRAY_PARTITION variable = bottleneck_0_dw_lower_buffer complete
	...
	const int bottleneck_0_first_fill_offset = chain_0_1_first_filter_dim
			- chain_0_1_first_strides;
	const int bottleneck_0_extra_cols_filled_first_time =
			chain_0_1_first_filter_dim - chain_0_1_first_strides;
...
#pragma HLS ARRAY_PARTITION variable = bottleneck_1_dw_lower_buffer complete
	...
	int filling_row = filling_h * chain_0_1_rows_filled_each_time
			+ chain_0_1_extra_rows_filled_first_time;
	//-------------------------------------------------------------------------
#if HW == CPU
	chain_0_1_fill_channels_buffer_cpu(channels,
	...
	for (filling_h; filling_h <= chain_0_1_ofms_height; filling_h++) {
		...
		filling_row = filling_h * chain_0_1_rows_filled_each_time
				+ chain_0_1_extra_rows_filled_first_time;
		//-------------------------------------------------------------------------
#if HW == CPU
		chain_0_1_fill_channels_buffer_cpu(channels,

...
#pragma HLS PIPELINE
		fms_dt expansion_results_buffer[bottleneck_expansion_parallelism_h
...
#pragma HLS UNROLL
				if (starting_h + p_h < bottleneck_ifms_height + padding_top) {
					expansion_results_buffer[p_h
							* bottleneck_expansion_parallelism_w + p_w] =
							pw_relu_norm(
									expansion_kernel(bottleneck_input,
		...
		fill_dw_ifms_buffer_upper_part(dw_input_buffer, previous_pass_dw_input,
		...
		fms_dt dw_result = dw_relu_norm(
				dw_kernel(dw_input_buffer, dw_weights, dw_filter_dim, d_in_out),

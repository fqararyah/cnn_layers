...
void _5_layer_0_3x3_conv(
	fms_dt channels_buffer[input_image_depth][layer_0_s_filter_dim + (_5_stages_layer_1_rows_at_once - 1) * layer_0_strides][input_image_width],
...
#pragma HLS INLINE off
	...
	fms_dt intermediate_channels_buffer[input_image_depth][layer_0_s_filter_dim + (_5_stages_layer_1_rows_at_once - 1) * layer_0_strides][layer_0_s_filter_dim] = {0};
#pragma HLS ARRAY_PARTITION variable = intermediate_channels_buffer type = complete dim = 0
	...
	{
		for (int h = 0; h < layer_0_s_filter_dim + (_5_stages_layer_1_rows_at_once - 1) * layer_0_strides; h++)
		{
...
#pragma HLS UNROLL
			...
			if (w < input_image_width - layer_0_strides)
			{
				...
				{
#pragma HLS UNROLL
					for (int c_h = 0; c_h < layer_0_s_filter_dim + (_5_stages_layer_1_rows_at_once - 1) * layer_0_strides; c_h++)
					{
#pragma HLS UNROLL
...
layer_1_pw__dw_main_loop:
		 ...
		 h++)
	{
			 ...
			 o_o_d++)
		{
			...
			{
#pragma HLS UNROLL
				for (int h = 0; h < layer_1_dw_filter_size - layer_1_dw_strides; h++)
...
#pragma HLS UNROLL
				  //  padding
					...
					{
#pragma HLS UNROLL
						intermediate_pw_results[d][h][w] = upper[o_o_d_offset + d][h][w - layer_1_dw_padding_left];
					}
				}
...
#pragma HLS UNROLL
							...
							if (scaled_val > 0)
							{
								result[o_o_d_offset + o_d][(w + 1 - (layer_1_dw_filter_size - layer_1_dw_padding_left)) / layer_1_dw_strides] =
									scaled_val;
							}
							//#####################end DW################
...
#pragma HLS UNROLL
								...
								if (c_h < layer_1_dw_filter_size - layer_1_dw_strides)
								{
...
#pragma HLS UNROLL
										if (w < layer_1_dw_ifm_width)
										{
											intermediate_pw_results[o_d][c_h][c_w] = upper[o_d][c_h][w + layer_1_dw_padding_left];
										}
										else
										{
						...
						//#####################shift#################
						for (int c_h = 0; c_h < layer_1_dw_filter_size - 2 * layer_1_dw_strides; c_h++)
						{
#pragma HLS UNROLL
							for (int c_w = 0; c_w < layer_1_dw_strides; c_w++)
							{
#pragma HLS UNROLL
								upper[o_o_d_offset + o_d][c_h][(w + 1 - (layer_1_dw_filter_size - 1)) + c_w] =
									upper[o_o_d_offset + o_d][c_h + layer_1_dw_strides][(w + 1 - (layer_1_dw_filter_size - layer_1_dw_padding_left)) + c_w];
							}
						}
						for (int c_h = 0; c_h < layer_1_dw_strides; c_h++)
						{
#pragma HLS UNROLL
							for (int c_w = 0; c_w < layer_1_dw_strides; c_w++)
							{
#pragma HLS UNROLL
								upper[o_o_d_offset + o_d][c_h + layer_1_dw_filter_size - 2 * layer_1_dw_strides]
								[(w + 1 - (layer_1_dw_filter_size - layer_1_dw_padding_left)) + c_w] =
									lower[o_o_d_offset + o_d][c_h][(w + 1 - (layer_1_dw_filter_size - layer_1_dw_padding_left)) + c_w];
							}
						}
						//#####################end shift#################
					}
...
void _5_stages_fill_channels_buffer(
	...
	fms_dt channels_buffer_0[input_image_depth][layer_0_s_filter_dim + (_5_stages_layer_1_rows_at_once - 1) * layer_0_strides][input_image_width],
	int starting_h)
{
...
#pragma HLS PIPELINE
		...
		{
#pragma HLS UNROLL
			for (int h = 0; h < layer_0_s_filter_dim + (_5_stages_layer_1_rows_at_once - 1) * layer_0_strides - layer_0_strides; h++)
			{
#pragma HLS UNROLL
...
#pragma HLS PIPELINE
		...
		{
#pragma HLS UNROLL
			for (int h = layer_0_s_filter_dim + (_5_stages_layer_1_rows_at_once - 1) * layer_0_strides - layer_0_strides; h < layer_0_s_filter_dim + (_5_stages_layer_1_rows_at_once - 1) * layer_0_strides; h++)
			{
#pragma HLS UNROLL
				channels_buffer_0[d][h][w] = channels[d][starting_h + h - (layer_0_s_filter_dim + (_5_stages_layer_1_rows_at_once - 1) * layer_0_strides - layer_0_strides)][w];
			}
		}
	}
}
...
#pragma HLS ARRAY_PARTITION variable = channels type = complete dim = 1
	...
	dw_weights_dt dw_weights_1[layer_1_dw_depth][layer_0_s_filter_dim][layer_0_s_filter_dim];

#pragma HLS ARRAY_PARTITION variable = dw_weights_1 type = complete dim = 1

	layer_0_weights_dt weights_0[layer_0_s_num_fils][layer_0_s_depth][layer_1_dw_filter_size][layer_1_dw_filter_size];
	...
	//#########################even###############################
	fms_dt channels_buffer_0[input_image_depth][layer_0_s_filter_dim + (_5_stages_layer_1_rows_at_once - 1) * layer_0_strides][input_image_width] = {0};
...
#pragma HLS ARRAY_PARTITION variable = _5_layer_2_pw_out_0 cyclic factor = layer_3_pw_parallelism_in / 2 dim = 1
	...
	//#########################odd###############################
	fms_dt channels_buffer_1[input_image_depth][layer_0_s_filter_dim + (_5_stages_layer_1_rows_at_once - 1) * layer_0_strides][input_image_width] = {0};

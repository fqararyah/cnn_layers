...
#pragma HLS INLINE off
	...
	fms_dt intermediate_channels_buffer[input_image_depth][layer_1_s_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides][v1_layer_2_dw_specs.filter_size] = {0};
#pragma hls array_PARTITION variable = intermediate_channels_buffer type = complete dim = 0
	...
	{
		for (int h = 0; h < layer_1_s_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides; h++)
		{
...
#pragma HLS UNROLL
			...
			if (w < input_image_width - layer_0_strides)
			{
				...
				{
#pragma HLS UNROLL
					for (int c_h = 0; c_h < layer_1_s_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides; c_h++)
					{
#pragma HLS UNROLL
...
#pragma hls array_PARTITION variable = intermediate_pw_results type = complete dim = 0
				 ...
				 o_d++)
			{
				...
				{
#pragma HLS UNROLL
					for (int h = 0; h < v1_layer_2_dw_specs.filter_size - v1_layer_2_dw_specs.strides - row; h++)
...
#pragma HLS UNROLL
					  //  padding
						...
						{
#pragma HLS UNROLL
							intermediate_pw_results[d][h][w] = upper[o_d_offset + d][h + row][w - v1_layer_2_dw_specs.padding_left];
						}
					}
					for (int h = v1_layer_2_dw_specs.filter_size - v1_layer_2_dw_specs.strides - row; h < v1_layer_2_dw_specs.filter_size; h++)
					{ // fill first two rows as the third will be filled by the pw results
#pragma HLS UNROLL
					  //  padding
						intermediate_pw_results[d][h][0] = 0;
						...
						{
#pragma HLS UNROLL
							intermediate_pw_results[d][h][w] = lower[o_d_offset + d][h - (v1_layer_2_dw_specs.filter_size - v1_layer_2_dw_specs.strides - row)]
																	[w - v1_layer_2_dw_specs.padding_left];
						}
					}
...
#pragma HLS UNROLL
						...
						if (scaled_val > 0)
						{
							result[o_d_offset + d][row][w / layer_2_dw_specs.strides] =
								scaled_val;
						}
					}
				}
...
#pragma HLS UNROLL
						...
						if (scaled_val > 0)
						{
							if (w + 1 < layer_2_dw_specs.filter_size)
							{
...
#pragma HLS INLINE off
	...
	int h_offset = starting_h < v1_4_stages_layer_1_rows_at_once * layer_0_strides ? layer_0_strides : layer_1_s_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides - (layer_1_s_filter_dim - layer_0_strides);
...
#pragma HLS PIPELINE
		...
		{
#pragma HLS UNROLL
			for (int h = 0; h < layer_1_s_filter_dim - layer_0_strides; h++)
			{
#pragma HLS UNROLL
...
#pragma HLS PIPELINE
		...
		{
#pragma HLS UNROLL
			for (int h = layer_1_s_filter_dim - layer_0_strides; h < layer_1_s_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides; h++)
			{
#pragma HLS UNROLL
				if (starting_h + h - (layer_1_s_filter_dim - layer_0_strides) < input_image_height)
				{
					channels_buffer_0[d][h][w] = channels[d][starting_h + h - (layer_1_s_filter_dim - layer_0_strides)][w];
				}
			}
		}
	}
...
#pragma HLS INLINE off
	...
	v1_4_fill_layers_weights(weights_1, dw_weights_2, dw_weights_2, pw_weights_3);

	//#########################even###############################
	fms_dt channels_buffer_0[input_image_depth][layer_1_s_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides][input_image_width] = {0};

	fms_dt v1_4_layer_0_3x3_conv_out_0[v1_layer_2_pw_depth][v1_4_stages_layer_1_rows_at_once][v1_layer_2_pw_ifm_width] =
		{0};

	fms_dt v1_4_layer_1_dw_upper[v1_layer_1_dw_depth][v1_layer_2_dw_specs.filter_size - v1_layer_2_dw_specs.strides][v1_layer_1_dw_ifm_width] = {0};
...
#pragma hls array_PARTITION variable = v1_4_layer_1_dw_out_0 complete dim = 2
	...
	//#########################odd###############################
	fms_dt channels_buffer_1[input_image_depth][layer_1_s_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides][input_image_width] = {0};
...
#pragma hls array_PARTITION variable = v1_4_layer_1_dw_out_1 complete dim = 2
	...
	v1_4_layer_0_3x3_conv(channels_buffer_0, weights_1,
	...
	v1_4_layer_0_3x3_conv(channels_buffer_1, weights_1,
						  v1_4_layer_0_3x3_conv_out_1);
	v1_4_layer_1_dw(v1_4_layer_1_dw_upper, v1_4_layer_0_3x3_conv_out_0, dw_weights_2, v1_4_layer_1_dw_out_0, 0);
	//##########
	v1_4_stages_fill_channels_buffer(channels, channels_buffer_1, 10);
	v1_4_layer_0_3x3_conv(channels_buffer_0, weights_1,
						  v1_4_layer_0_3x3_conv_out_0);
	v1_4_layer_1_dw(v1_4_layer_1_dw_upper, v1_4_layer_0_3x3_conv_out_1, dw_weights_2, v1_4_layer_1_dw_out_1, 1);
	//##########
	int even_odd = 1;
	int h = 4;
main_pipeline_loop:
	for (; h < switch_point_fms_height; h++)
	{
		if (even_odd)
		{
			...
			v1_4_layer_0_3x3_conv(channels_buffer_1, weights_1,
								  v1_4_layer_0_3x3_conv_out_1);
			v1_4_layer_1_dw(v1_4_layer_1_dw_upper, v1_4_layer_0_3x3_conv_out_0, dw_weights_2, v1_4_layer_1_dw_out_0, 1);
			v1_4_layer_2_pw_dw(v1_4_layer_1_dw_out_1, pw_weights_3, dw_weights_2, v1_4_layer_2_dw_upper,
		...
		else
		{
			...
			v1_4_layer_0_3x3_conv(channels_buffer_0, weights_1,
								  v1_4_layer_0_3x3_conv_out_0);
			v1_4_layer_1_dw(v1_4_layer_1_dw_upper, v1_4_layer_0_3x3_conv_out_1, dw_weights_2, v1_4_layer_1_dw_out_1, 1);
			v1_4_layer_2_pw_dw(v1_4_layer_1_dw_out_0, pw_weights_3, dw_weights_2, v1_4_layer_2_dw_upper,
							   v1_4_layer_2_dw_lower, result, h - 4, 1);
		}
		even_odd = 1 - even_odd;
	}

...
#pragma HLS INLINE off
	...
	fms_dt intermediate_channels_buffer[input_image_depth][first_conv_layer_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides][v1_layer_2_dw_specs.filter_size] = {0};
#pragma hls array_PARTITION variable = intermediate_channels_buffer type = complete dim = 0
	...
	{
		for (int h = 0; h < first_conv_layer_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides; h++)
		{
...
#pragma HLS UNROLL
			...
			if (w < input_image_width - layer_0_strides)
			{
				...
				{
#pragma HLS UNROLL
					for (int c_h = 0; c_h < first_conv_layer_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides; c_h++)
					{
#pragma HLS UNROLL
...
#pragma HLS INLINE off
	...
	int h_offset = starting_h < v1_4_stages_layer_1_rows_at_once * layer_0_strides ? layer_0_strides : first_conv_layer_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides - (first_conv_layer_filter_dim - layer_0_strides);
...
#pragma HLS PIPELINE
		...
		{
#pragma HLS UNROLL
			for (int h = 0; h < first_conv_layer_filter_dim - layer_0_strides; h++)
			{
#pragma HLS UNROLL
...
#pragma HLS PIPELINE
		...
		{
#pragma HLS UNROLL
			for (int h = first_conv_layer_filter_dim - layer_0_strides; h < first_conv_layer_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides; h++)
			{
#pragma HLS UNROLL
				if (starting_h + h - (first_conv_layer_filter_dim - layer_0_strides) < input_image_height)
				{
					channels_buffer_0[d][h][w] = channels[d][starting_h + h - (first_conv_layer_filter_dim - layer_0_strides)][w];
				}
			}
		}
	}
...
#pragma HLS INLINE off
	...
	//#########################even###############################
	fms_dt channels_buffer_0[input_image_depth][first_conv_layer_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides][input_image_width] = {0};
...
#pragma hls array_PARTITION variable = v1_4_layer_1_dw_out_0 complete dim = 2
	...
	//#########################odd###############################
	fms_dt channels_buffer_1[input_image_depth][first_conv_layer_filter_dim + (v1_4_stages_layer_1_rows_at_once - 1) * layer_0_strides][input_image_width] = {0};

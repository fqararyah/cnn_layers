...
#pragma HLS INLINE off
	...
	fms_dt intermediate_channels_buffer[input_image_depth][first_conv_layer_filter_dim + (v1_3_stages_layer_1_rows_at_once - 1) * layer_0_strides][first_conv_layer_filter_dim] = {0};
#pragma HLS ARRAY_PARTITION variable = intermediate_channels_buffer type = complete dim = 0
	...
	{
		for (int h = 0; h < first_conv_layer_filter_dim + (v1_3_stages_layer_1_rows_at_once - 1) * layer_0_strides; h++)
		{
...
layer_0_ofms:
	for (int o_o_d = 0;
		 o_o_d < first_conv_layer_num_fils / v1_sesl_layer_0_parallelism_ofms; o_o_d++)
	{
		// outer filters loop
...
#pragma HLS UNROLL
			...
			if (w < input_image_width - layer_0_strides)
			{
				...
				{
#pragma HLS UNROLL
					for (int c_h = 0; c_h < first_conv_layer_filter_dim + (v1_3_stages_layer_1_rows_at_once - 1) * layer_0_strides; c_h++)
					{
#pragma HLS UNROLL
...
void v1_3_stages_fill_channels_buffer(
	...
	fms_dt channels_buffer_0[input_image_depth][first_conv_layer_filter_dim + (v1_3_stages_layer_1_rows_at_once - 1) * layer_0_strides][input_image_width],
	int starting_h)
{
...
#pragma HLS PIPELINE
		...
		{
#pragma HLS UNROLL
			for (int h = 0; h < first_conv_layer_filter_dim + (v1_3_stages_layer_1_rows_at_once - 1) * layer_0_strides - layer_0_strides; h++)
			{
#pragma HLS UNROLL
...
#pragma HLS PIPELINE
		...
		{
#pragma HLS UNROLL
			for (int h = first_conv_layer_filter_dim + (v1_3_stages_layer_1_rows_at_once - 1) * layer_0_strides - layer_0_strides; h < first_conv_layer_filter_dim + (v1_3_stages_layer_1_rows_at_once - 1) * layer_0_strides; h++)
			{
#pragma HLS UNROLL
				channels_buffer_0[d][h][w] = channels[d][starting_h + h - (first_conv_layer_filter_dim + (v1_3_stages_layer_1_rows_at_once - 1) * layer_0_strides - layer_0_strides)][w];
			}
		}
	}
}
...
#pragma HLS INLINE off
	...
	layer_0_weights_dt weights_1[first_conv_layer_num_fils][first_conv_layer_depth][first_conv_layer_filter_dim][first_conv_layer_filter_dim];
...
#pragma HLS ARRAY_PARTITION variable = dw_weights_2 type = complete dim = 1
	...
	//#########################even###############################
	fms_dt channels_buffer_0[input_image_depth][first_conv_layer_filter_dim + (v1_3_stages_layer_1_rows_at_once - 1) * layer_0_strides][input_image_width] = {0};
...
#pragma HLS ARRAY_PARTITION variable = v1_3_layer_1_dw_out_0 cyclic factor = 2 dim = 2
	...
	//#########################odd###############################
	fms_dt channels_buffer_1[input_image_depth][first_conv_layer_filter_dim + (v1_3_stages_layer_1_rows_at_once - 1) * layer_0_strides][input_image_width] = {0};

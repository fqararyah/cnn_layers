...
layer_0_ofms:
	for (int o_o_d = 0;
		 o_o_d < layer_0_num_fils / v1_layer_0_parallelism_ofms; o_o_d++)
	{
		// outer filters loop
		int o_o_d_offset = o_o_d * v1_layer_0_parallelism_ofms; // for indexing in depth
...
#pragma HLS PIPELINE
		...
		layer_0_parallelized_ofms:
			for (int o_d = 0;
				 o_d < v1_layer_0_parallelism_ofms; o_d++)
			{

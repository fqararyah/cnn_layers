#include "fpga_cpp_fc.h"

void read_ifms(string file_name, int8_t ifms[]) {
	int a;
...
void read_fc_weights(string file_name, int8_t fc_weights[]) {
	int a;
	std::ifstream infile(file_name);
	assert(!infile.fail());
	bool failed = false;
...
void read_weight_sums(string file_name, int64_t fc_weight_sums[]) {
	int a;
	std::ifstream infile(file_name);
	assert(!infile.fail());
	bool failed = false;
...
void read_biases(string file_name, int fc_biases[]) {
	int a;
	std::ifstream infile(file_name);
	assert(!infile.fail());
	bool failed = false;
...
void fc_layer(fms_dt in_vector[], int8_t weights[], int64_t weight_sums[],
		int top5[5], int biases[], const fc_layer_specs layer_specs_struct) {
	...
	int32_t sum_vector[4] = { 0 };
	for (int i = 0; i < num_classes; i++) {
		int64_t pss = 0;
		const int row_start_index = i * fc_layer_input_size;
//        for (int j = 0; j < fc_layer_input_size; j++)
//        {
//            pss += (int64_t)weights[row_start_index + j] * in_vector_int8[j];
//        }
		int32x4_t targetv = { 0 };
		for (int j = 0; j < fc_layer_input_size; j += 16) {
			/* Load data into NEON register */
			int8x8_t av1 = vld1_s8(&(in_vector_int8[j]));
			int8x8_t av2 = vld1_s8(&(in_vector_int8[j + 8]));
			int8x8_t bv1 = vld1_s8(&(weights[row_start_index + j]));
			int8x8_t bv2 = vld1_s8(&(weights[row_start_index + j + 8]));

			int16x8_t tmp1 = vmull_s8(av1, bv1);
			int16x8_t tmp2 = vmull_s8(av2, bv2);

			int32x4_t tmp3 = vpaddlq_s16(tmp1);
			int32x4_t tmp4 = vpaddlq_s16(tmp2);

			targetv = vaddq_s32(targetv, tmp3);
			targetv = vaddq_s32(targetv, tmp4);
		}
		vst1q_s32(sum_vector, targetv);

		pss = sum_vector[0] + sum_vector[1] + sum_vector[2] + sum_vector[3];
		// if(i==999)cout << pss <<" "<<weight_sums[i]<<" "<<biases[i]<<"\n";
		pss_vector[i] = pss
				+ (-weight_sums[i] * layer_specs_struct.ifm_zero_point)
				+ biases[i];
	}
#else
	for (int i = 0; i < num_classes; i++)
	...
	for (int i = 0; i < 5; i++) {
		...
		for (int j = 0; j < num_classes; j++) {
			if (pss_vector[j] > max && top5[0] != j && top5[1] != j
					&& top5[2] != j && top5[3] != j && top5[4] != j) {
				max = pss_vector[j];
				top5[i] = j;
			}
		}

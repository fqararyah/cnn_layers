...
with open(h_file, 'w') as wf:
    ...
    to_generate_for_layers = cgc.LAST_LAYER_TO_GENERATE if  cgc.LAST_LAYER_TO_GENERATE > 0 else len(model_dag)
    num_of_generated_for_layers = 0
    for layer_index in range(to_generate_for_layers):
        layers_fused_parameters_offsets[layer_index + 1] = layers_fused_parameters_offsets[layer_index]
        ...
        if layer_type == 'pw':
            weights = np.reshape(weights,(layer_weight_shape[0], layer_weight_shape[1]))
        elif layer_type == 'dw':
            weights = np.reshape(weights,(layer_weight_shape[0], layer_weight_shape[1], layer_weight_shape[2]))
        else:
            weights = np.reshape(weights,(layer_weight_shape[0], layer_weight_shape[1], layer_weight_shape[2], layer_weight_shape[3]))
        ...
        if (cgc.PIPELINE == False or num_of_generated_for_layers >= cgc.PIPELINE_LEN):
            layers_fused_parameters_offsets[layer_index + 1] += layer_weight_shape[0]

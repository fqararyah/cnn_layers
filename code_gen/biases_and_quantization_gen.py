...
skip_connections_indices = utils.get_skip_connections_indices()
...
with open(h_file, 'w') as wf:
    ...
    for layer_index in range(overall_fms_scales):
        if expansion_projection[layer_index - skip_connection_current_index] == 0:
            continue
        ...
        if layer_index - skip_connection_current_index in skip_connections_indices:
    ...
    for layer_index in range(len(layers_weights_shapes)):
        if expansion_projection[layer_index] == 0:
            continue

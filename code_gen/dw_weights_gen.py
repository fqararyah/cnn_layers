...
with open(dw_weights_h_file, 'w') as f:
    ...
    for ii in range(first_pipeline_layer, last_pipeline_layer):
    ...
    if cgc.PIPELINE == False or last_layer > last_pipeline_layer:
        ...
        for ii in range(first_layer, last_layer):
            if layers_types[ii] != 'dw':
                dw_layers_weights_offsets.append(dw_layers_weights_offsets[-1])
                continue
            dw_layers_weights_offsets.append(
                dw_layers_weights_offsets[-1] + layers_weights[ii].num_of_filters)
            weights_file = weights_files_location + \
                reading_weights_file_format.format(str(ii))
            with open(weights_file, 'r') as f2:
                for i in range(layers_weights[ii].num_of_filters):
